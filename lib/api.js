import { SELECTED_GAMES, TOP_GAMES } from "./constants";
...
export const getGameBySlug = async (slug) => {
  const data = await fetchAPI(
    `
    query getGameBySlug ($where: String,  $include: [String]) {
      games (filter: { slug: { _eq: $where }, gid: { _in: $include} }) {
        title
        category {
          name
          slug
...
export const getAllGamesWithSlug = async (limit) => {
  const data = await fetchAPI(
    `
    query getAllGamesWithSlug ($where: Int, $include: [String]) {
      games (filter: { gid: { _in: $include} }, limit: $where) {
        slug
      }
    }
  `,
...
export const getGamesByCategory = async (category, limit) => {
  const data = await fetchAPI(
    `
    query getGamesByCategory ($where: String, $limit: Int, $include: [String]) {
      games (filter: { category: { slug: { _eq: $where } }, gid: { _in: $include} }, limit: $limit,) {
        title
        slug
        gid
        category {
  ...
  `,
    {
      variables: {
        ...
        include: TOP_GAMES.length ? TOP_GAMES : [],
      },
    }
  );
  return data?.games;
